...
def run_cli_launcher():
    ...
    if setup_embedded_python():
        ...
        print(f"🚀 Launching: {' '.join(cmd)}")
        try:
            result_code, output = run_command_live(cmd, env=env, cwd=BASE_DIR)
            if result_code != 0:
                dump = write_failure_dump(
                    "cli",
    ...
    method = test_portablemc(sys_python)
    if not method:
        dump = write_failure_dump("cli", "portablemc unavailable after system Python install.", extra=failure_context)
        if dump:
            print(f"📝 Failure dump written: {dump}")
