...
def get_system_python():
    ...
    for ver in versions:
        ...
        common_paths.append(
            f"C:\\Users\\{os.environ.get('USERNAME')}\\AppData\\Local\\Programs\\Python\\Python{ver.replace('.', '')}\\python.exe"
        )
        common_paths.append(
            f"C:\\Program Files\\Python{ver.replace('.', '')}\\python.exe"
        )
        common_paths.append(
            f"C:\\Program Files (x86)\\Python{ver.replace('.', '')}\\python.exe"
        )
